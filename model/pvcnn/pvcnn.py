...
# import pdb
from model.CoAttention import CoAttention
...
class PVCNN2Base(nn.Module):
    def __init__(
        ...
        self.pc_attention = MultiheadAttention(n_features, num_heads)
        self.vl_id_attention = MultiheadAttention(n_features, num_heads)
        self.cs_attention = MultiheadAttention(n_features, num_heads)
        ...
        # self.coattention = CoAttention(pose_features_dim, pc_features_dim, hidden_dim)
    ...
    def forward(self, inputs: torch.Tensor, t: torch.Tensor, x_0_vl_id: torch.Tensor, P: int, keypoints):
        """
        ...
        # x_0_vl_id = x_0_vl_id.unsqueeze(2)
        # x_0_vl_id = x_0_vl_id.repeat(1, 1, P, 1)
        # x_0_vl_id = x_0_vl_id.reshape((B, -1, 3))
        # x_0_vl_id = x_0_vl_id.permute(0, 2, 1)
        #
        # x_0_vl_id = x_0_vl_id.permute(0, 2, 1)
        features = features.permute(0, 2, 1)
        features, _ = self.pc_attention(features, features, features)
        vl, _ = self.vl_id_attention(x_0_vl_id, x_0_vl_id, x_0_vl_id)
        features, _ = self.cs_attention(features, vl, vl)
        features = features.permute(0, 2, 1)

        # features = torch.cat((features, x_0_vl_id), dim=2)
        # Downscaling layers
        coords_list = []
        in_features_list = []
        ...
        # Output MLP layers
        # features = self.coattention(keypoints, features.permute(0, 2, 1)).permute(0, 2, 1)
        output = self.classifier(features)  # B, 3, 2000
...
if __name__ == '__main__':
    ...
    tensor = torch.rand(32, 2048, 3).to('cuda:0')
    output = model(tensor)
    print()