...
def get_optimizer(config, model: torch.nn.Module) -> torch.optim.Optimizer:
    ...
    print('weight decay = {weight_decay}'.format(weight_decay=weight_decay))
    ...
    # Construct optimizer
    if config['optimizer']['type'] == 'torch':
        Optimizer: torch.optim.Optimizer = getattr(torch.optim, config['optimizer']['name'])
        optimizer = Optimizer(parameters, lr=lr)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer,
                                                    step_size=config['optimizer']['step_size'],
                                                    gamma=config['optimizer']['gamma'])
    else:
        raise NotImplementedError(f'Invalid optimizer config')

    return optimizer, scheduler


class MetricLogger(object):
    def __init__(self, delimiter="\t", save_log_path=None):
        ...
        if self.save_log_path is not None:
            if not os.path.exists(self.save_log_path):
                with open(self.save_log_path, 'w'):
                    pass
